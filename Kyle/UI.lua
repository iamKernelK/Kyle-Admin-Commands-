...
-- مسح الواجهة القديمة إذا قمت بتشغيل السكربت أكثر من مرة
...
-- جلب الأوامر من الرابط الخاص بك
local Commands = {}
...
-- الإطار الرئيسي (الواجهة)
local MainFrame = Instance.new("Frame")
MainFrame.Size = UDim2.new(0, 400, 0, 0) -- مغلق بالبداية
...
local function UpdateList(filter)
    ...
    for cmdName, cmdData in pairs(Commands) do
        if filter == "" or string.find(string.lower(cmdName), string.lower(filter)) then
            ...
            Item.Text = cmdName .. " " .. (cmdData.Description or "")
